...
baseline_workflow = build_baseline_workflow()

# 외부(API엔드포인트) 에서 호출하는 함수 초기 공책(initial_state)를 받아서
# 워크플로우를 실행하고 최종결과 TriageResponse를 반환
def run_baseline_workflow(initial_state: OpsAgentState) -> TriageResponse:
    final_state = baseline_workflow.invoke(initial_state)
